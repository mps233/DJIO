...
struct ATURC: Sendable, Equatable {
  ...
  init(raw: String) {
    ...
    self.raw = normalized
    guard normalized.uppercased().hasPrefix("+CMTI:") else {
      storage = nil
      index = nil
      return
    }
...
final class ATTransportWorker: ATTransporting, @unchecked Sendable {
  ...
  private let queue = DispatchQueue(label: "com.cellularbridge.at-transport", qos: .userInitiated)
  private let readerQueue = DispatchQueue(
    label: "com.cellularbridge.at-transport.reader", qos: .userInitiated)
  ...
  init(
    stateQueue: DispatchQueue = DispatchQueue(label: "com.cellularbridge.at-transport.state")
  ) {
  ...
  private static func isUnsolicitedLine(_ line: String) -> Bool {
    ...
    return uppercased.hasPrefix("+CMTI:") || uppercased.hasPrefix("+CDSI:")
      || uppercased == "RING" || uppercased == "NO CARRIER"
