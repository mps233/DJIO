...
struct ATResponseParserTests {
  ...
  @Test func parsesOnlyIncomingVoiceCallsFromCLCC() {
    let response = """
      ...
      +CLCC: 2,1,4,0,0,"8613800138000",145\r\n
      +CLCC: 3,1,5,0,0\r\n
      +CLCC: 4,1,4,1,0,"ignored-data",129\r\n
      OK\r\n
      """

    #expect(
      parser.incomingVoiceCalls(from: response) == [
        ATIncomingVoiceCall(modemIdentifier: 2, callerNumber: "+8613800138000"),
        ATIncomingVoiceCall(modemIdentifier: 3, callerNumber: nil),
      ])
  }

  @Test func ignoresMalformedCLCCLines() {
    #expect(parser.incomingVoiceCalls(from: "+CLCC: broken\r\nOK").isEmpty)
    #expect(parser.incomingVoiceCalls(from: "+CLCC: 1,1,6,0,0,\"10086\",129\r\nOK").isEmpty)
  }
}
